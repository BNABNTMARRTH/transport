...
## ADR-0017 — Adaptation to ReversePort (Vanilla HTML5 / WebGL Three.js + Node.js)

**Context.** ReversePort is a production reverse tunnel service with a high-performance 3D WebGL landing page and a native Node.js core. It does not use React/Next.js/Svelte; it utilizes native HTML5, design tokens in `website/style.css`, and a Three.js 0.162.0 GLTF engine (`website/octopus3d.js`).

**Decision.**
1. Configure `framework.id: "vanilla"`, `language: "js"`, and `packageManager: "npm"`.
2. Map `paths.source` and `paths.views` to `website/`, `paths.styles` to `website/style.css`, and `paths.server` to `server/`.
3. Set `bindings.motion` to `three` for the interactive 3D octopus model.
4. Allow discrete `@keyframes` for pure utility indicators (spinner loader, terminal cursor blink) while enforcing that interactive motion and 3D remain spring-driven and physics-backed.
5. Deactivate `routesDelegateToViews` and `serverFirst` checks to align with the standalone multi-page HTML and client-side WebGL canvas architecture.

**Consequences.** The mechanical gate `verify.sh` passes with 0 FAILs, all 13 AI skills are activated for the workspace, and the project adheres strictly to three-tier design tokens, clean API envelopes, and 3D scene performance guidelines.
